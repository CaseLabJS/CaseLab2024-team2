...
function getTypeReferences(folders) {
    const references = {};
...
function loadJsonValues(filePath) {
    const rawData = fs.readFileSync(filePath, 'utf8');
    return JSON.parse(rawData);
}
// Замена unknown на вложенные типы с корректным путем импорта
function replaceUnknownTypes(content, folderPath, references, jsonValues) {
    const imports = new Set(); // Хранение импортов для файла
