...
export const getDocType = async (id: number) => {
...
export const addDocType = async (createTypesDoc: DocTypesCreate) => {
...
export const deleteDocType = async (id: number) => {
  const response = await api.delete(`/document_types/${id}`);
  return response.data;
};
